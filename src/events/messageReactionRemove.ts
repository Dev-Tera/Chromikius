        ...
        for (const selfrole of selfroles) {
            try {
                ...
                if (err.code == RESTJSONErrorCodes.UnknownRole) {
                    ...
                    embed = await createSelfroleEmbed("Mir fehlt die Berechtigung die Rolle der Selfrole zu entfernen", [selfrole], reaction.message.guild)
                } else {
                    console.error("messageReactionRemove: Couldn't remove selfrole")
                    console.error(err)
                }

                if (embed != undefined) {
                   const moderationChannel = await reaction.message.guild.channels.fetch(Config.guild.moderationChannelId).catch((err) => {
                       console.error("messageReactionRemove:MissingPermissions: Couldn't fetch moderationChannel")
                   }) 
